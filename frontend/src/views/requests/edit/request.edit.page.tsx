...
const RequestEditPage = observer(() => {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<RequestsDto.RequestUpdateForm>({
    resolver: zodResolver(RequestsDto.RequestUpdateForm),
  });
  ...
  const onSubmit = async (data: RequestsDto.RequestUpdateForm) => {
    ...
    <>
      <Link to={`/requests/${id}`}>
        <Stack align={"center"} gap={6}>
          ...
          <Text size={16}>К информации заявки</Text>
        </Stack>
      </Link>
